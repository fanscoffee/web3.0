...
function Header() {
  ...
  return (
    <header className="fixed left-0 top-0 w-full bg-neutral-100 text-center text-neutral-600 dark:bg-neutral-600 dark:text-neutral-200 lg:text-left">
      <div className="items-center justify-between px-7 py-4 md:flex md:px-10">
        ...
        <ul
          className={`absolute left-0 z-[-1] mt-5 w-full pb-12 pl-1 md:static md:z-auto md:flex md:w-auto md:items-center md:pb-0 md:pl-0 ${
            open ? 'top-30 opacity-100' : 'top-[-490px]'
          } opacity-0 md:opacity-100 `}
        >
          {links.map((link) => (
            <li key={link.name} className="my-7 text-xl md:my-0 md:ml-8">
              <a href={link.link} className=" hover:text-neutral-500">
                {link.name}
              </a>
            </li>
          ))}
